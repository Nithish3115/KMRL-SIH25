...
class DataService:
    ...
    """
    def __init__(self, file_path='train_schedule_data.csv'):
        self.file_path = file_path
        self.df = None
        self.label_encoders = {}
        self.cleaning_schedule = self._load_json_data('cleaning_schedule.json', default={"default": {}})
        self.branding_contracts = self._load_branding_contracts()
        # NEW: Load the depot layout at startup
        self.depot_layout = self._load_json_data('depot_layout.json', default={}).get('depot_layout', {})
    ...
    def _load_branding_contracts(self):
        ...
        contracts_data = self._load_json_data('branding_contracts.json', default={}).get("contracts", [])
    ...
    def get_track_properties(self, track_name: str) -> dict:
        ...
        """
        # Return a default value for safety if a track name is somehow invalid
        return self.depot_layout.get(track_name, {"accessibility_score": 0, "has_maintenance_access": False, "status": "Unknown"})
    
    # ... (All other methods remain stable and correct) ...
    ...
    def preprocess_for_prediction(self, daily_data):
        df = daily_data.copy()
        if not self.label_encoders: self.label_encoders = joblib.load('label_encoders.pkl')
        categorical_cols = ['job_card_status', 'branding_priority', 'cleaning_status', 'stabling_location']
        for col in categorical_cols:
